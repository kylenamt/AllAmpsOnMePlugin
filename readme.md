...
- A NAM (Neural Amp Modeler) network, trained in junction with an embedding table for each .nam capture produces a general purpose NAM network that can produces different amp profile by selecting the right embedding.
- The network can produce hundreds of audio profiles (405 in this first iteration) solely by updating the embedding—a 128-dimension array of fp32 numbers—along with its corresponding metadata.
